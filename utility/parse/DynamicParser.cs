...
public class DynamicParser
{
  ...
  public Level ParseLevel(JObject json,out Level.LevelBuilder skeleton)
  {
	...
	{
	  Npc npc=DynamicClone<JObject,Npc>(token, null, 
	...
	List<ObjectiveDisplayGroup> displayGroups = ((JArray)dyn.objectiveDisplayGroups ?? new JArray())
	  ...
	  {
		return DynamicParse<JObject,ObjectiveDisplayGroup>(token,
 ...
 */
  private TV DynamicParseHelper<TV,TP>(JToken token,JObject extraPointers, Func<TP,TV> parser,
    out bool isUnparsedPointer) where TP:JToken
  {
	isUnparsedPointer = false;
	if (token.Type == JTokenType.String && filteredPointers.TryGetValue(token.Value<string>(), out var value))
	{
	  GD.Print("gotcha: "+value);
	  return (TV)value;
	}

	return parser(OrOfPointer<TP>(token, extraPointers, out isUnparsedPointer));
  }
 ...
 */
  public TV DynamicParse<TP, TV>(JToken token,JObject extraPointers,Func<TP,TV> parser) 
	where TP:JToken
  {
    var value = DynamicParseHelper(token, extraPointers, parser, out var isPointer);
 ...
 * resulting value to filteredPointers.
 */
  public TV DynamicClone<TP, TV>(JToken token,JObject extraPointers,Func<TP,TV> parser)
	where TP:JToken where TV:class,ICloneable<TV>
  {
	var value = DynamicParseHelper(token, extraPointers, parser, out var isPointer)?.Clone();
